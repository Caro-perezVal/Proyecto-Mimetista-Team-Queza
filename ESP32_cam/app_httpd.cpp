...
// ==========================================
// NUESTRO HANDLER PERSONALIZADO
// ==========================================
extern int red;
extern int green;
extern int blue;
extern int cuadrante_silueta;
extern bool actualizarHardware;

static esp_err_t custom_control_handler(httpd_req_t *req) {
    char buf[128];
    if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
        char var[32];
        if (httpd_query_key_value(buf, "r", var, sizeof(var)) == ESP_OK) red = atoi(var);
        if (httpd_query_key_value(buf, "g", var, sizeof(var)) == ESP_OK) green = atoi(var);
        if (httpd_query_key_value(buf, "b", var, sizeof(var)) == ESP_OK) blue = atoi(var);
        if (httpd_query_key_value(buf, "pos", var, sizeof(var)) == ESP_OK) {
            int pos = atoi(var);
            if (pos >= 1 && pos <= 5) cuadrante_silueta = pos;
        }
        actualizarHardware = true;
    }
...
void startCameraServer()
{
    ...
    httpd_uri_t custom_control_uri = {
        .uri = "/custom_control",
        .method = HTTP_GET,
        .handler = custom_control_handler,
        .user_ctx = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        // ¡REGISTRAMOS NUESTRA NUEVA RUTA AQUÍ!
        httpd_register_uri_handler(camera_httpd, &custom_control_uri);
    }
